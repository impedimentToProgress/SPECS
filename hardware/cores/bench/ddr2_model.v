...
`include "ddr2_model_parameters.v"
   ...
   // Number of address bits needed to address all bursts
    `define MAX_BITS   (BA_BITS+ROW_BITS+COL_BITS-BL_BITS)
   // Number of burts of maximum size that the memory needs to store
...
`endif
    ...
    task memory_write;
       ...
       begin
          ...
          // chop off the lowest address bits to get the base address
	  // for this burst
          addr = {bank, row, col} >> BL_BITS;
...
`else
	    ...
	    else if (memory_used > `MEM_SIZE) begin
